...
Every admitted output purpose, including saved-reference tests and generated-candidate previews, reveals the overlay on its first submitted speech frame. This is triggered by actual native output rather than clicking Play, waiting for generation, lead-in or background-only tails. The existing speaking ribbon consumes the same fresh samples for previews and replies. Showing the overlay does not request focus, change microphone state or grant readiness. The overlay is revealed once per output; hiding it during that output is respected until the next utterance.
...
The media worker reduces only the reference's valid submitted samples to32 bounded stereo-energy RMS values, at most20 sample events per second. Averaging squared channel samples avoids phase cancellation and signed-wave averaging hiding audible output; original playback references remain unchanged. The display lane retains no waveform history. The separate native [Personal audio](personal-audio.md) lane retains at most one second of actual mixed stereo reference before display throttling, for provisional residual processing only. References older than250ms are discarded. A reference's original monotonic submission time gives a fixed250ms display expiry; receiving an event never renews it. Clear events are consumed even without clock calibration. Native sequence exhaustion disables this observation lane rather than wrapping or affecting effect authority.

Signal rendering subscribes to every fresh frame even when the WebGL drawing callback is installed after the initial reactive effect. During fresh speech, browser animation frames advance the ribbon phase using the current measured envelope; they never invent samples or extend the frame expiry. Hidden, cleared, expired or retired output stops animation. Reduced motion retains the existing frozen-per-output presentation.

The speaking ribbon maps measured energy through a bounded display curve, `1 - exp(-4 * energy)`, capped at 94% of its available height. This gives quieter phrases more visible movement without changing playback gain or turning silence into an invented signal. Other capture/thinking displays keep their existing scale.
