...
The finite native portrait operations advance the shared telemetry.db writer to schema 8 (schema 7 extended the preferences command roster; schema 6 introduced the app timing tables); accepted trace query version 3 and product Store schema 30 are separate boundaries. Install matched Windows/controller binaries built from this source. Writers supporting at most schema 7 reject schema 8, so rolling back only an executable will make telemetry unavailable; preserve the old telemetry backup or keep the newer writer. Do not reset, rename or delete the owner/conversation store as a telemetry workaround. The new installation-scoped app records are local-only and are not added to the paired accepted-trace query. Static/build checks do not prove the upgrade or retained export on an installed instance.
