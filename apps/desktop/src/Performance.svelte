<script lang="ts">
  ...
  type Snapshot = { uptime_seconds: number; capacity: number; retained: number; evicted: number; observer_loss: number; active_outputs: number; silent_diagnostic: boolean; summaries: Row[] };
...
<section class="section">
  ...
  <p class="av-hint">Actual preview, greeting and explicit live activity measurements from this companion process. Opening this view starts no recording, playback or model request.</p>
  {#if snapshot}
    ...
    {#if snapshot.summaries.length === 0}<div class="empty"><h2>No observed sessions yet</h2><p class="av-hint">Explicitly use a voice preview or live activity check to collect measurements. Pre-admission failures and batch checks are outside this view.</p></div>{/if}
