...
<section class="section">
  ...
  <div class="av-card p-4"><h3>Gmail account</h3><p class="av-hint">Uses your saved Gmail site permission. Opening messages may mark them read. Nothing is sent, archived or deleted.</p>
    {#if gmailScopes.length>1}<label class="av-label" for="gmail-browser">Browser</label><select id="gmail-browser" class="av-select" bind:value={scope} disabled={busy}><option value="">Choose your Gmail browser</option>{#each gmailScopes as item}<option value={item.id}>{item.origin}</option>{/each}</select>{/if}
