<script lang="ts">
  ...
  type Page = { task:string; origin:string; blocks:string[]; truncated:boolean; excluded_content:boolean; remaining_ms:number; provider:ProviderProbe|null; x_ready:{account:string;focused:boolean}|null; x_needs_input:{account:string;reason:"login_required"|"account_mismatch"|"unsupported_page"}|null };
...
<section class="section">
  ...
  <div class="av-card p-4"><h3>X account</h3><p class="av-hint">Uses your paired browser and saved X site permission. Nothing is posted or typed.</p>
    ...
    {#if xScopes.length>1}<label class="av-label" for="x-browser">Browser</label><select id="x-browser" class="av-select" bind:value={scope} disabled={busy}><option value="">Choose your X browser</option>{#each xScopes as item}<option value={item.id}>{item.origin}</option>{/each}</select>{/if}
  ...
  <div class="av-card p-4"><h3>Optional endpoint investigation</h3><p class="av-hint">Say “Diagnose download” with the existing Computer performance read grant for local CPU, interfaces, default routes and DNS configuration. Select a particular source below only when you want endpoint probes. Other details are optional and remain owner-reported.</p>
    ...
    <label class="av-label" for="download-port">Source port</label><select id="download-port" class="av-select" bind:value={downloadPort} disabled={busy}><option value={443}>443</option><option value={80}>80</option></select>
    ...
    <label class="av-label" for="download-unit">Displayed unit</label><select id="download-unit" class="av-select" bind:value={downloadUnit} disabled={busy}><option value="megabytes_per_second">MB/s (decimal)</option><option value="megabits_per_second">Mbps</option><option value="bytes_per_second">bytes/s</option></select>
    <label class="av-label" for="download-throttle">App throttle visibly enabled?</label><select id="download-throttle" class="av-select" bind:value={downloadThrottle} disabled={busy}><option value="unknown">Unknown</option><option value="yes">Yes, observed by me</option><option value="no">No, observed by me</option></select>
    </details>
  ...
  {#if !snapshot}<p class="av-hint">Refresh to inspect durable task state.</p>{:else if snapshot.tasks.length === 0}<p class="av-hint">No accepted action tasks for the current owner.</p>{:else}{#each snapshot.tasks as task}<div class="row"><div><h3>{task.payload.kind === "diagnose_download" ? `Diagnose ${targetName(task)}` : task.payload.kind === "flush_download_dns" ? `Clear DNS cache for diagnosis of ${targetName(task)}` : task.payload.kind === "connect_vpn" ? `Connect ${targetName(task)}` : task.payload.kind === "read_page" ? `Read partial page at ${targetName(task)}` : task.payload.kind === "fill_prompt" ? `Prepare unsubmitted draft in ${targetName(task)}` : task.payload.kind === "diagnostic" ? targetName(task) : task.payload.kind === "set_volume" ? `Set ${targetName(task)} to ${task.payload.percent}%` : `Open ${targetName(task)}`}</h3><p class="av-hint">{label(task)} · {new Date(task.created_ms).toLocaleString()}</p>
