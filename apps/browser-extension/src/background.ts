...
async function connect() {
  ...
  try {
    ...
    async function authenticate() {
      ...
      send({ type: "authenticate", body: { version: 8, session: challenge.session, challenge: challenge.challenge, pairing: record.pairing, proof } });
    }
    async function receive(value: unknown) {
      ...
      try {
        ...
        } else if (p.object(value, ["type", "version", "challenge", "pairing", "credential"]) && value.type === "issued" && value.version === 8 && p.challenge(value.challenge) && p.pairing(value.pairing) && p.hex(value.credential)) {
    ...
    send({ type: "hello", body: { version: 8, installation, connection, extension: chrome.runtime.id, nonce, pairing: savedPairing } });
  } catch {
    if (generation === epoch) {
