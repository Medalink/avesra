...
export type Challenge = { version: 8; installation: string; connection: string; session: string; challenge: string; nonce: string; pairing: Pairing | null };
...
export type Status = { version: 8; session: string; generation: number; sequence: number; state: "pending" | "authenticated_no_scopes"; authority: Authority | null; scope: Scope | null; document: DocumentRequest | null; read: ReadRequest | null; read_ack: ReadContext | null };
...
export function status(value: unknown): value is Status {
  return object(value, ["version", "session", "generation", "sequence", "state", "authority", "scope", "document", "read", "read_ack"])
    && value.version === 8 && id(value.session) && counter(value.generation) && counter(value.sequence)
...
export function challenge(value: unknown): value is Challenge {
  return object(value, ["version", "installation", "connection", "session", "challenge", "nonce", "pairing"]) && value.version === 8 && id(value.installation) && id(value.connection) && id(value.session) && id(value.challenge) && hex(value.nonce) && (value.pairing === null || pairing(value.pairing));
}
...
export async function comparison(value: Challenge) {
  const text = ["AVESRA-BROWSER-COMPARE-8", value.installation, value.connection, value.session, value.challenge, value.nonce, ""].join("\n");
...
export async function proof(value: Challenge, record: Saved, clientNonce: string) {
  ...
  try {
    ...
    const text = ["AVESRA-BROWSER-AUTH-8", record.pairing.id, record.pairing.revision, value.installation, value.connection, value.session, clientNonce, value.nonce, chrome.runtime.id, ""].join("\n");
