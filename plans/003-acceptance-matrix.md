...
| Explicit redraw | Missing; changed-source read currently asks for an unavailable operation | Authorized redraw succeeds, preserves the actor ring, and rejects stale owner/device/window/proof contexts |
| Optional twenty-word capture | Unfinished work excluded from integration | Real two-batch capture, missing/ambiguous slots, cancellation, selected-device ownership and worker retirement |
| Measured portrait features | Version 1 has absent petals and tempo | Actual native extraction and versioned storage; prompt pacing must not be reported as measured speech rhythm |
...
| Native timing | IPC round-trip only | Bounded phase records from actual derivation/publication/capture/extraction, including failure and withdrawal |
