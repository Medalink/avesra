...
| M1 | Establish workspace, contracts, build/static gates, and configuration (no tests per owner override) | M0 | PARTIAL — latest Windows Static independently passed at `290554f`; latest full bundled-asset release Build remains `32c241e`; shared core/server passed independent Spark ARM64 Static at `543316e`, with executor ARM release confirmed in its log; remaining contracts/telemetry incomplete |
...
| M5 | Control Chrome/Brave and desktop/CLI prompt inputs | M4 | IN PROGRESS — extension native-port ownership, dormant bounded local pipe transport, typed protocol and native credential/handshake helpers through `290554f` reviewed and statically checked; native owner confirmation/coordinator and authenticated endpoint wiring remain incomplete; browser operations and native prompt drivers remain unavailable |
