...
| M1 | Establish workspace, contracts, build/static gates, and configuration (no tests per owner override) | M0 | PARTIAL — latest Windows Static, Spark ARM64 Static and full Windows bundled-asset release Build independently passed at `ccc4055` (source `c69cee9`), with executor ARM release confirmed in its log; remaining contracts/telemetry incomplete |
