...
| M1 | Establish workspace, contracts, build/static gates, and configuration (no tests per owner override) | M0 | PARTIAL — Windows and Spark ARM64 Static independently passed at `afc058d`; Windows bundled-asset release Build independently passed at `95a84b1`; executor ARM release passed at `afc058d`; remaining contracts/telemetry incomplete |
...
| M3 | Prove local speech, speaker enrollment, intent gating, and playback | M2 | IN PROGRESS — batch speaker/ASR supervisors loaded but unqualified; final TTS load observed then stopped; protected candidate selection, recording/paired inference, stable endpoint binding and typed playback remain uninvoked; incremental ASR supervisor/client source through `afc058d` is compiled but undeployed, with paired streaming ingress, identity activation and live voice proof incomplete |
