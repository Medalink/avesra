...
| M1 | Establish workspace, contracts, build/static gates, and configuration (no tests per owner override) | M0 | PARTIAL — Windows Static and Spark ARM64 Static independently passed at docs-only `4af2894` (source `6b21ac9`), with executor ARM release confirmed in its log; latest full Windows bundled-asset release Build independently passed at docs-only `56089d5` (source `8bc7b23`); remaining contracts/telemetry incomplete |
...
| M4 | Ship overlay, settings, onboarding, and volume/app launch slice | M3 | PARTIAL — approved design port, native settings/tray/local controls inspected; source includes native volume adapter and durable worker ownership with typed observations through `95a84b1`; generated-voice management and design-matched Settings through `11580d0`, plus native shortcuts/editor through `6b21ac9`, pass Static; native effects, shortcuts, setup and new UI remain uninvoked; onboarding, app launch and usable action ingress incomplete |
