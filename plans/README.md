...
Current installed Windows and controller: **49cf90c**, 2026-09-26, Store30 / telemetry8 / query3. Desktop SHA-256 was rechecked as `a3aa5a9ab471864d1525209130ec7bb6fd5b06d61f7f21ee8a197eaaa86ce040`. The real People card recovered owner status only after Retry; that loading failure is retained. Actual native avatar-source lookup timings exported successfully, but no populated owner avatar or Overlay timing rows were observed. Reviewed source `c623a84` adds the serialized People-read fix, optional continuous-input portrait observer/UI, exact mock phrase-chip colors and timing-state startup correction; its installed proof is pending. Earlier Save/Discard and custom preview evidence keeps its own build identity. Full parity remains unproven. Plan001 remains
...
| [003](003-voice-avatar.md) | Draw each enrolled person's voice as a reproducible avatar matching the authoritative mock | P2 | M, staged | Plan 001 protected owner and enrollment flow | IN PROGRESS — protected native parameters, redraw and optional portrait observer/UI are reviewed and integrated. Installed49 proves only the unavailable card, owner-read failure/recovery and native source-lookup timings. The newer People-read and Overlay startup fixes await installed proof. Genuine avatar/capture/restart, live matched activity, additional people and authentic mock comparison remain open. See the [acceptance matrix](003-acceptance-matrix.md). |
