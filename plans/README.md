...
| M1 | Establish workspace, contracts, build/static gates, and configuration (no tests per owner override) | M0 | PARTIAL — latest Windows Static and Spark ARM64 Static independently passed at `e15e525`, with executor ARM release confirmed in its log; latest full Windows bundled-asset release Build remains `ccc4055` (source `c69cee9`); remaining contracts/telemetry incomplete |
