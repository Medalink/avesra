...
| --- | --- | --- | --- | --- | --- |
| [001](001-single-spark-assistant.md) | Build and verify Avesra on Spark2 with Local Studio, a Windows companion, optional client GPU acceleration and per-stage metrics | P1 | L, multiple milestones | Current-state reconciliation C0; scoped hardware/app qualification | IN PROGRESS / PARTIAL — Matched Windows/ARM `c4e0f14` is installed after Static/release checks and an observed22→24 database migration. It includes the startup metadata fix, native clock/facts, Gmail transport and accepted voice timing. Actual UI/empty trace reads passed; initial model probing exposed a frontend race being fixed separately. Real accepted conversation remains unproven. A real isolated preview and empty trace read/export passed at75b5de0; these do not prove task effects. Required live owner workflows and A01-A29 remain open; see the [current acceptance ledger](001-acceptance-2026-09-26.md). |
