...
| --- | --- | --- | --- | --- | --- |
| [001](001-single-spark-assistant.md) | Build and verify Avesra on Spark2 with Local Studio, a Windows companion, optional client GPU acceleration and per-stage metrics | P1 | L, multiple milestones | Current-state reconciliation C0; scoped hardware/app qualification | IN PROGRESS / PARTIAL — Windows startup hotfix `5bde1f5` is installed with unchanged matching shared ARM `75b5de0`. It corrects an observed activity-metadata blocker; real accepted conversation remains unproven. The next clock/facts/Gmail-transport/timing source checkpoint passes combined Static but is not yet deployed. A real isolated preview and empty trace read/export passed at75b5de0; these do not prove task effects. Required live owner workflows and A01-A29 remain open; see the [current acceptance ledger](001-acceptance-2026-09-26.md). |
