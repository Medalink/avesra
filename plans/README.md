...
| M1 | Establish workspace, contracts, build/static gates, and configuration (no tests per owner override) | M0 | PARTIAL — latest Windows Static independently passed at `f3e66a8`; latest full bundled-asset release Build passed at source `dc32f98`; shared core/server passed independent Spark ARM64 Static at `f3e66a8`, with executor ARM release confirmed in its log; remaining contracts/telemetry incomplete |
| M2 | Pair PC/Spark; implement task policy, persistence, and cancellation | M1 | PARTIAL — TLS control, ordered ledger, exact approvals/cancellation/reconciliation, pairing recovery and bounded paired WSS ASR/speaker ingress through `2051e82`; native producer remains dormant; original-budget opaque conversation storage, schema-5 recovery and same-worker acceptance/status/cancel helpers through `f3e66a8` reviewed and passed independent Windows/ARM Static; actual PC pairing, qualified runtime acceptance, planner/task linkage and live effect/recovery proof remain incomplete |
