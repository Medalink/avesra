...
| M1 | Establish workspace, contracts, build/static gates, and configuration (no tests per owner override) | M0 | PARTIAL — latest Windows Static independently passed at `a9b3e86`; latest full bundled-asset release Build passed at source `ee35025`; shared core/server passed independent Spark ARM64 Static at `42939a2`, with executor ARM release confirmed in its log; remaining contracts/telemetry incomplete |
...
| M5 | Control Chrome/Brave and desktop/CLI prompt inputs | M4 | IN PROGRESS — typed local pairing transport, native-host/extension persistence and Settings confirmation UI through `ee35025` reviewed against approved design source and passed full Windows Static/release Build; immutable selected-installation storage and protected Settings controls through `a9b3e86` reviewed and passed full Windows Static, with the catalog-failure recovery correction in `95e41fe` source-reviewed; paths remain uninvoked and rendered parity unverified; selected connection lifetime, origin grants, page operations and native prompt drivers remain incomplete |
