...
| M1 | Establish workspace, contracts, build/static gates, and configuration (no tests per owner override) | M0 | PARTIAL — latest Windows Static independently passed at `f849559`; latest full bundled-asset release Build passed at source `1bfd6bb`; shared core/server passed independent Spark ARM64 Static at `f849559`, with executor ARM release confirmed in its log; remaining contracts/telemetry incomplete |
...
Reasoning adapter checkpoint: bounded SSE parsing, strict configured Docker-incarnation metadata and durable single-job uncertainty bookkeeping through `f849559` are source-reviewed and pass independent Windows/ARM Static. A dropped caller or restart cannot clear outstanding backend work. Actual loaded-artifact qualification, HTTP ownership and public planner integration remain incomplete; these helpers have not made requests or opened runtime storage.
