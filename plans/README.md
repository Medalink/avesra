...
| M1 | Establish workspace, contracts, build/static gates, and configuration (no tests per owner override) | M0 | PARTIAL — latest Windows Static and full bundled-asset release Build independently passed at source `a1a7286`; shared core/server passed independent Spark ARM64 Static at `87f2cfd`, with executor ARM release confirmed in its log; remaining contracts/telemetry incomplete |
