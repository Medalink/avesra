...
| M1 | Establish workspace, contracts, build/static gates, and configuration (no tests per owner override) | M0 | PARTIAL — latest Windows Static independently passed at `42939a2`; latest full bundled-asset release Build remains `32c241e`; shared core/server passed independent Spark ARM64 Static at `42939a2`, with executor ARM release confirmed in its log; remaining contracts/telemetry incomplete |
...
| M5 | Control Chrome/Brave and desktop/CLI prompt inputs | M4 | IN PROGRESS — extension native-port ownership, bounded local pipe transport, typed credential/handshake helpers and explicit Settings pairing coordinator through `42939a2` reviewed and statically checked; coordinator remains uninvoked; native-host/extension relay and design-matched confirmation UI under implementation; browser operations and native prompt drivers remain unavailable |
