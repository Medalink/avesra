...
| M1 | Establish workspace, contracts, build/static gates, and configuration (no tests per owner override) | M0 | PARTIAL — latest Windows Static independently passed at `d6cb0e4`; latest full bundled-asset release Build passed at source `d6cb0e4`; shared core/server passed independent Spark ARM64 Static at `d6cb0e4`, with executor ARM release confirmed in its log; remaining contracts/telemetry incomplete |
...
| M5 | Control Chrome/Brave and desktop/CLI prompt inputs | M4 | IN PROGRESS — native/extension pairing, immutable browser selection, protected Settings controls and separately owned selected connection through `ddd4a9e` reviewed against approved design source; UI recovery/status corrections through `ab7ff3d` reviewed; browser v3 status and connection-bound expiring observation context through `7e6884e` passed full Windows Static and shared-contract ARM verification; exact-origin scope setup/recovery, v4 native/extension handshake and design-referenced approval controls through `d6cb0e4` reviewed and passed full Windows Static and independent ARM Static; full bundled Windows release at `d6cb0e4` passed; paths remain uninvoked and rendered parity unverified; actual permission/grant setup remains uninvoked; page operations and native prompt drivers remain incomplete |
