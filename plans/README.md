...
| --- | --- | --- | --- | --- | --- |
| [001](001-single-spark-assistant.md) | Build and verify Avesra on Spark2 with Local Studio, a Windows companion, optional client GPU acceleration and per-stage metrics | P1 | L, multiple milestones | Current-state reconciliation C0; scoped hardware/app qualification | IN PROGRESS / PARTIAL — Matched Windows/ARM `ebcc9b1` is installed after Static/release checks. The earlier22→24 migration was observed. Native clock/facts, Gmail transport and accepted voice timing are installed. Actual fresh startup now loads model status without Refresh; the frontend probe race is repaired. Actual UI/empty trace reads passed. Real accepted conversation remains unproven. A real isolated preview and empty trace read/export passed at75b5de0; these do not prove task effects. Required live owner workflows and A01-A29 remain open; see the [current acceptance ledger](001-acceptance-2026-09-26.md). |
