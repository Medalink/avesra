...
| M1 | Establish workspace, contracts, build/static gates, and configuration (no tests per owner override) | M0 | PARTIAL — latest Windows Static independently passed at `26583d6`; latest full bundled-asset release Build remains `32c241e`; unchanged shared core/server passed independent Spark ARM64 Static at `b26cb47`, with executor ARM release confirmed in its log; remaining contracts/telemetry incomplete |
...
| M5 | Control Chrome/Brave and desktop/CLI prompt inputs | M4 | IN PROGRESS — authenticated browser bridge and connection/document contracts starting; browser operations and native prompt drivers remain unavailable |
