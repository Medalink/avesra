...
| M1 | Establish workspace, contracts, build/static gates, and configuration (no tests per owner override) | M0 | PARTIAL — latest Windows Static independently passed at docs-only `c7e8f1a` (source `d53a911`); Spark ARM64 Static passed at docs-only `c003c2f` (source `073564b`), with executor ARM release confirmed in its log and no subsequent shared-core/server changes; latest full Windows bundled-asset release Build remains `1ecbbff` (source `37f3113`); remaining contracts/telemetry incomplete |
