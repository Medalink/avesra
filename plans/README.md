...
| M1 | Establish workspace, contracts, build/static gates, and configuration (no tests per owner override) | M0 | PARTIAL — latest Windows Static independently passed at docs-only `40f587e` and Spark ARM64 Static at docs-only `c003c2f` (both source `073564b`), with executor ARM release confirmed in its log; latest full Windows bundled-asset release Build remains `1ecbbff` (source `37f3113`); remaining contracts/telemetry incomplete |
...
| M4 | Ship overlay, settings, onboarding, and volume/app launch slice | M3 | PARTIAL — approved design port, native settings/tray/local controls inspected; source includes native volume adapter through `95a84b1`, immutable app catalog/executable launch through `3fb18ae`, bounded shortcut/App Paths discovery through `37f3113`, and protected owner creation/design-matched People card through `073564b`; generated-voice management through `11580d0` and shortcuts/editor through `6b21ac9` pass Static/release Build; owner checkpoint passes Static, with enrollment management follow-through under review; native effects, discovery, shortcuts, setup and new UI remain uninvoked; focus/package support, app setup/onboarding and usable action ingress incomplete |
