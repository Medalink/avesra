...
The compact overlay is transparent and built around one live signal bar driven by real microphone/playback levels and lane events, drawn as one set of centred voice bars. Colour and pulse carry the state: human audio in the colour of its transient source classification (owner blue `#3A5DD8`, other people green `#8EDE4A`, background grey), Avesra thinking as low, slow light-Ruby bars, and Avesra speaking as tall Ruby bars. State changes reshape and recolour the same bars in a centre-out ripple of about 400 ms (see the Transitions section of `design/mockups/OverlayStates.dc.html`). Working, muted/deafened, paused, and disconnected use their own flat indicators. Source coloring is a live display only: it creates no retained record of unknown speakers and is neither an identity decision nor a permission. Draw the bars with WebGL, with a plain CSS fallback of the same geometry and colours; provide a static reduced-motion frame, and stop animating while hidden.
